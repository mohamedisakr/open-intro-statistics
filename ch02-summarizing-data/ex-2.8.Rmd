...
total_population_each_county <- county %>% 
  group_by(name) %>% 
  summarise(total_population = sum(pop2017)) %>% arrange(desc(total_population))
...
county_name_list <- n_distinct(county$name)
county_name_list

# weighted_mean = as.numeric( weighted.mean(county$median_hh_income, county$pop2017,  na.rm = TRUE)) * 1L # county_name_list ,

weighted_mean = as.double( weighted.mean(county$median_hh_income, county$pop2017,  na.rm = TRUE)) * 1.0 # county_name_list ,
